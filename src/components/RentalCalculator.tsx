...
export default function RentalCalculator() {
  ...
  const totalPrice = useMemo(() => {
    return selectedCar ? totalDays * selectedCar.price : 0
  ...
  return (
    <section className="py-20 px-6 max-w-3xl mx-auto">
      ...
      <form className="grid gap-5 bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-2xl shadow-sm">
        ...
        <button
          type="submit"
          className="mt-2 px-6 py-3 bg-white text-black rounded-full font-semibold hover:bg-[#d4af37] hover:text-white transition"
        >
          Оставить заявку
        </button>
      </form>
